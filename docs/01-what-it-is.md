...
排查这类驻留组件时，第一个坑不是技术，是**名字**。同一个产品在服务、驱动、进程、安装目录、计划任务里用四个不同的名字，很容易把三个不同的东西当成一个。
...
> **因为混淆，我在排查时踩过一次坑**：一度把 `aliedit` 的服务（`pcas`、`secbizsrv`）也算进"AlibabaProtect 没能删干净"的证据里。它们确实是阿里系组件，但**属于另一个产品**，两者的处置路径不同。
