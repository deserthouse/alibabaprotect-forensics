...
**简体中文** · [English](05-execution-forensics.en.md)
...
**对照组**：同一次触发中，**上游更新器**（未被拦截）**有** Prefetch 记录 —— 这进一步确认"该触发确实发生过"，而不是笔者误判了触发时机。
...
> 这个实验的价值在于：**它不是"笔者认为它会生效"，而是"笔者让它试了一次，看着它失败"**。任何机制类结论都应该做到这一步。
